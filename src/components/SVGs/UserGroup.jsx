import React from 'react';

const UserGroup = () => {
  return (
    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
      <g id="vuesax/bold/profile-2user">
        <g id="profile-2user">
          <path
            id="Vector"
            d="M13.5 3C9.57 3 6.375 6.195 6.375 10.125C6.375 13.98 9.39 17.1 13.32 17.235C13.44 17.22 13.56 17.22 13.65 17.235C13.68 17.235 13.695 17.235 13.725 17.235C13.74 17.235 13.74 17.235 13.755 17.235C17.595 17.1 20.61 13.98 20.625 10.125C20.625 6.195 17.43 3 13.5 3Z"
            fill="white"
          />
          <path
            id="Vector_2"
            d="M21.12 21.225C16.935 18.435 10.11 18.435 5.895 21.225C3.99 22.5 2.94 24.225 2.94 26.07C2.94 27.915 3.99 29.625 5.88 30.885C7.98 32.295 10.74 33 13.5 33C16.26 33 19.02 32.295 21.12 30.885C23.01 29.61 24.06 27.9 24.06 26.04C24.045 24.195 23.01 22.485 21.12 21.225Z"
            fill="white"
          />
          <path
            id="Vector_3"
            d="M29.985 11.01C30.225 13.92 28.155 16.47 25.29 16.815C25.275 16.815 25.275 16.815 25.26 16.815H25.215C25.125 16.815 25.035 16.815 24.96 16.845C23.505 16.92 22.17 16.455 21.165 15.6C22.71 14.22 23.595 12.15 23.415 9.90003C23.31 8.68503 22.89 7.57503 22.26 6.63003C22.83 6.34503 23.49 6.16503 24.165 6.10503C27.105 5.85003 29.73 8.04003 29.985 11.01Z"
            fill="white"
          />
          <path
            id="Vector_4"
            d="M32.985 24.885C32.865 26.34 31.935 27.6 30.375 28.455C28.875 29.28 26.985 29.67 25.11 29.625C26.19 28.65 26.82 27.435 26.94 26.145C27.09 24.285 26.205 22.5 24.435 21.075C23.43 20.28 22.26 19.65 20.985 19.185C24.3 18.225 28.47 18.87 31.035 20.94C32.415 22.05 33.12 23.445 32.985 24.885Z"
            fill="white"
          />
        </g>
      </g>
    </svg>
  );
};

export default UserGroup;
